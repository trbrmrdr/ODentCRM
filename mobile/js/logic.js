$(document).ready(function () {
    $('.login-btn').click(function () {
        $('.welcome-screen').removeClass('active-screen');
        $('.login-screen').addClass('active-screen');
    ...
    $('.reg-btn').click(function () {
        $('.welcome-screen').removeClass('active-screen');
        $('.reg-screen').addClass('active-screen');

        return false;
    });

    $('.menu-link, .back-btn').click(function () {
        if (!$(this).hasClass('menu-logo')) {
            if ($(this).hasClass('menu-link')) {
                $('.menu-link.active-link').removeClass('active-link');
                $(this).addClass('active-link');
            }
            var target_screen = $(this).attr('target-screen');
            $('.active-screen').removeClass('active-screen');
            $('.' + target_screen).addClass('active-screen');
            if (target_screen == 'chat-screen' || target_screen == 'login-screen' || target_screen == 'welcome-screen') {
                $('.main-menu').hide();
            } else {
                $('.main-menu').show();
            }
    ...
    $('.start-btn').click(function () {
        var login = $('.login-screen input[name="login-phone"]').val();
        var pass = $('.login-screen input[name="login-pass"]').val();

        if (login == '') {
            $('.login-screen input[name="login-phone"]').addClass('error');
        }
        if (pass == '') {
            $('.login-screen input[name="login-pass"]').addClass('error');
        }
    ...
    $('.start-reg-btn').click(function () {
        var login = $('.reg-screen input[name="login-phone"]').val();
        var pass = $('.reg-screen input[name="login-pass"]').val();

        if (login == '') {
            $('.reg-screen input[name="login-phone"]').addClass('error');
        }
        if (pass == '') {
            $('.reg-screen input[name="login-pass"]').addClass('error');
        }
    ...
    $('input[name="login-phone"]').mask('+7 (999) 999-99-99');

    $('.one-tab-link').click(function () {
        $('.one-tab-link.active').removeClass('active');
        $(this).addClass('active');
        var target_tab = $(this).attr('target-tab');
        $('.one-tab.active-tab').removeClass('active-tab');
        $('#' + target_tab + '-tab').addClass('active-tab');

        return false;
    });

    $('.alert-icon').click(function () {
        $('#alert_alert').modal('show');

        return false;
    });

    $('.one-main-slide').click(function () {
        $('.active-screen').removeClass('active-screen');
        $('.card-screen').addClass('active-screen');
    });

    $('.menu-logo').click(function () {
        $('#empty_alert').modal('show');

        return false;
    });

    $('.one-time').click(function () {
        var visit_time = $(this).find('.visit-time').text();
        var visit_date = $(this).find('.visit-date').text();
        var doctor_fio = $(this).find('.doctor-fio').text();
        var time_id = $(this).attr('time-id');

        $('#free_time_modal .free-time-doctor-fio').text(doctor_fio);
        $('#free_time_modal .free-time-date').text(visit_date);
        $('#free_time_modal .free-time-time').text(visit_time);
        $('#free_time_modal .reserve-time-btn').attr('time-id', time_id);

        $('#free_time_modal').modal('show');
    });

    $('.reserve-time-btn').click(function () {
        var time_id = $(this).attr('time-id');
        $('#free_time_modal').modal('hide');

        $('.one-time[time-id="' + time_id + '"]').addClass('reserved-time');

        return false;
    });

    $('.one-service').click(function () {
        var service_name = $(this).find('.service-name').text();
        var service_price = $(this).find('.service-price').text();

        $('#service_modal .modal-title').text(service_name);
        $('#service_modal .service-price').text(service_price);

        $('#service_modal').modal('show');
    });

    $('.one-tooth').click(function () {
        var tooth_name = $(this).find('.tooth-name').text();
        var tooth_desc = $(this).find('.tooth-desc').text();

        $('#tooth_modal .modal-title').text(tooth_name);
        $('#tooth_modal .tooth-desc').text(tooth_desc);

        $('#tooth_modal').modal('show');
    });
});
...
(function (t) {
    ...
    function me(n, r) {
        ...
        function z(t, e) {
            t = K.moment(t), e = e ? K.moment(e) : t.hasTime() ? t.clone().add(K.defaultTimedEventDuration) : t.clone().add(K.defaultAllDayEventDuration), oe.select({start: t, end: e})
        }
    ...
    var Re, ke, Me, Fe = ["sun", "mon", "tue", "wed", "thu", "fri", "sat"], ze = ["year", "month", "week", "day", "hour", "minute", "second", "millisecond"], Le = {}.hasOwnProperty, Pe = /^\s*\d{4}-\d\d$/,
        Ve = /^\s*\d{4}-(?:(\d\d-\d\d)|(W\d\d$)|(W\d\d-\d)|(\d\d\d))((T| )(\d\d(:\d\d(:\d\d(\.\d+)?)?)?)?)?$/, _e = e.fn, Ge = t.extend({}, _e);
    De.moment = function () {
        return A(arguments)
    ...
    Xe.mixin({
        mousedOverSeg: null, isDraggingSeg: !1, isResizingSeg: !1, segs: null, renderEvents: function (t) {
            ...
            var n, i = this, r = this.view, s = t.el, o = t.event, l = new We(t.el, {parentEl: r.el, opacity: r.opt("dragOpacity"), revertDuration: r.opt("dragRevertDuration"), zIndex: 2}), d = new Ie(r.coordMap, {
            ...
            r = new Ie(this.coordMap, {
                distance: 5, scroll: o.opt("dragScroll"), dragStart: function (e) {
                    ...
                    i = e.end, i.isAfter(h) || (i = h.clone().add(E(e.end, e.start))), i.isSame(f) ? i = null : l.isEventRangeAllowed({start: h, end: i}, c) ? (s.renderEventResize({
                        start: h,
                        end: i
            ...
            var e = this.view, n = t.source || {}, i = t.color, r = n.color, s = e.opt("eventColor"), o = t.backgroundColor || i || n.backgroundColor || r || e.opt("eventBackgroundColor") || s,
                l = t.borderColor || i || n.borderColor || r || e.opt("eventBorderColor") || s, a = t.textColor || n.textColor || e.opt("eventTextColor"), u = [];
    ...
    var Ue = Xe.extend({
        slotDuration: null, snapDuration: null, minTime: null, maxTime: null, axisFormat: null, dayEls: null, slatEls: null, slatTops: null, helperEl: null, businessHourSegs: null, constructor: function () {
            ...
            for (t = {start: t.start.clone().stripZone(), end: t.end.clone().stripZone()}, n = 0; s > n; n++) i = this.colData[n].day, r = {
    ...
    tn.duration = {months: 1}, Ce.basicWeek = {type: "basic", duration: {weeks: 1}}, Ce.basicDay = {type: "basic", duration: {days: 1}}, n({
        allDaySlot: !0,
        allDayText: "all-day",
        scrollTime: "06:00:00",
